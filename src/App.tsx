...
function Missions({data,persist,mission,selectedMission,setSelectedMission,goMission,goWell,setPage}:any){const[add,setAdd]=useState(false);const saveMission=(m:Mission)=>{persist({...data,missions:[...data.missions,m]});setAdd(false);setSelectedMission(m.id)}
 ...
 return <MissionDetail data={data} mission={mission} city={city} wells={wells} total={total} update={update} back={()=>{setSelectedMission(null);setPage('missions')}} goWell={goWell} onDelete={()=>{if(removeMission(mission))setSelectedMission(null)}}/>
}
