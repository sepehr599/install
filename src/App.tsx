...
export default function App() {
  ...
  const setTheme = () => { const next: AppData = { ...data, theme: data.theme === 'light' ? 'dark' : 'light' }; persist(next) }
...
function Missions({ data, persist, mission, selectedMission, setSelectedMission, goMission }: any) {
  ...
  const addMissionFiles = async (files: FileList | null) => {
    if (!files?.length) return
    const selected = Array.from(files)
    const items = await Promise.all(selected.map(file => new Promise<MediaItem>((resolve, reject) => {
      const reader = new FileReader()
      reader.onload = () => resolve({ id: id(), name: file.name, type: file.type.startsWith('image/') ? 'photo' : 'receipt', dataUrl: String(reader.result), createdAt: new Date().toISOString() })
      reader.onerror = () => reject(reader.error)
      reader.readAsDataURL(file)
    })))
    update({ ...mission, files: [...mission.files, ...items] })
  }
  ...
  return <>
    ...
    <div className="mission-detail-grid">
      ...
      <Section title="رسیدها و فایل‌های مأموریت" subtitle="عکس غذا، فاکتور، رسید و Screenshot اسنپ را می‌توانید یکجا نگه دارید">
        <div className="upload-box">
          <label className="secondary file-btn"><Upload size={17}/> افزودن فایل<input type="file" accept="image/*,application/pdf" multiple hidden onChange={e=>addMissionFiles(e.target.files)}/></label>
          <div className="mission-files">{mission.files.map(f=><div className="mission-file" key={f.id}>{f.url||f.dataUrl ? <a href={f.url||f.dataUrl} target="_blank" rel="noreferrer">{f.name}</a> : <span>{f.name}</span>}<button className="icon-btn" onClick={()=>update({...mission,files:mission.files.filter(x=>x.id!==f.id)})}>×</button></div>)}{!mission.files.length&&<p className="muted-text">هنوز فایلی ثبت نشده است.</p>}</div>
        </div>
      </Section>
    </div>
  </>
...
function Reports({data}:{data:AppData}) {
  const total=data.missions.reduce((sum,m)=>sum+(m.meal?.amount||0)+m.travel.reduce((a,t)=>a+t.amount,0)+m.otherExpenses.reduce((a,t)=>a+t.amount,0),0)
  const meal=data.missions.reduce((sum,m)=>sum+(m.meal?.amount||0),0)
  const travel=data.missions.reduce((sum,m)=>sum+m.travel.reduce((a,t)=>a+t.amount,0),0)
  const exportMissions=()=>downloadCsv('flowmeter-missions.csv',[
    ['تاریخ','عنوان','شهر','تعداد چاه','غذا','رفت و آمد','سایر','مجموع'],
    ...data.missions.map(m=>{
      const city=data.cities.find(c=>c.id===m.cityId)?.name||''
      const tr=m.travel.reduce((a,t)=>a+t.amount,0), other=m.otherExpenses.reduce((a,t)=>a+t.amount,0), mealAmount=m.meal?.amount||0
      return [m.date,m.title,city,String(m.wellIds.length),String(mealAmount),String(tr),String(other),String(mealAmount+tr+other)]
    })
  ])
  const exportWells=()=>downloadCsv('flowmeter-wells.csv',[
    ['شهر','نام چاه','کد','وضعیت','Latitude','Longitude','آخرین تاریخ','کیفیت سیگنال','قدرت سیگنال','Path','Flow L/s','Transmitter','Sensor'],
    ...data.wells.map(w=>{
      const city=data.cities.find(c=>c.id===w.cityId)?.name||''
      const snaps=data.snapshots.filter(s=>s.wellId===w.id).sort((a,b)=>b.date.localeCompare(a.date)), last=snaps[0]
      return [city,w.name,w.code,statusLabels[w.status],String(w.location?.latitude??''),String(w.location?.longitude??''),last?.date||'',String(last?.signalQuality??''),String(last?.signalPower??''),last?.soundPath||'',String(last?.flow??''),last?.transmitterSerial||'',last?.sensorSerial||'']
    })
  ])
  return <>
