...
export interface MediaItem { id: string; name: string; type: 'photo' | 'audio' | 'receipt' | 'screenshot' | 'invoice'; url?: string; dataUrl?: string; storagePath?: string; createdAt: string; duration?: number }
export interface InstallSnapshot {
  id: string; type: 'installation' | 'visit'; wellId: string; date: string; createdAt: string
  latitude?: number; longitude?: number; accuracy?: number
  pipeMaterial: string; pipeDiameter?: number; pipeThickness?: number; liningThickness?: number
  signalQuality?: number; signalPower?: number; soundPath?: 'Z' | 'V'
  transmitterSerial: string; sensorSerial: string; flow?: number; notes: string
  photos: MediaItem[]; voices: MediaItem[]
}
